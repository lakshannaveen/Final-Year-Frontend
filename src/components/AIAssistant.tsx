...
export default function AIAssistant({
  ...
  const renderProfilePicWithRing = (user: ServiceSuggestion["user"]) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 ${dark ? 'bg-black/70' : 'bg-black/50'} backdrop-blur-sm transition-all duration-300`}>
      <div className={`relative w-full sm:w-[600px] h-full sm:h-[700px] ${dark ? 'bg-slate-900' : 'bg-white'} sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden ${dark ? 'border border-slate-700' : ''}`}>
        {/* Header */}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className={`flex-1 overflow-y-auto p-3 sm:p-6 ${dark ? 'bg-gradient-to-b from-slate-900 to-slate-800' : 'bg-gradient-to-b from-gray-50 to-emerald-50'}`}>
            {/* Welcome and Quick Questions */}
            {chatHistory.length === 0 && !aiLoading && (
              <div className="text-center py-8 sm:py-12">
                <div className={`rounded-2xl p-6 sm:p-8 shadow-lg max-w-2xl mx-auto ${dark ? 'bg-slate-800 border-slate-700 text-gray-100' : 'bg-white border border-emerald-100'}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {questions.map((item, index) => (
                            <button
                              ...
                              className={`w-full text-left p-3 rounded-lg transition-all duration-200 text-sm font-medium active:scale-95 flex items-center gap-2 group ${dark ? 'bg-slate-800 hover:bg-slate-700 border border-slate-700 text-emerald-300 hover:shadow-md' : 'bg-white hover:bg-emerald-50 border border-emerald-200 text-emerald-800 hover:shadow-md hover:border-emerald-300'}`}
                            >
                  ...
                  <div className={`mt-6 p-4 rounded-lg ${dark ? 'bg-slate-700 border border-slate-700 text-gray-100' : 'bg-emerald-50 border border-emerald-200'}`}>
                  ...
                  <div className={`mt-6 p-4 rounded-lg ${dark ? 'bg-yellow-900/20 border border-yellow-800 text-yellow-200' : 'bg-yellow-50 border border-yellow-200 text-yellow-700 text-sm font-medium'}`}>
            ...
            <div className="space-y-3 sm:space-y-4">
                    ...
                    <div
                      className={`max-w-[90%] sm:max-w-[80%] rounded-2xl p-3 sm:p-4 shadow-sm ${
                        message.type === "user"
                          ...
                          : `${dark ? 'bg-slate-800 text-gray-100 border-slate-700' : 'bg-white text-gray-800 border-emerald-100'} rounded-bl-none border shadow-md`
                      }`}
                    >
                      <div className="flex items-start gap-2 sm:gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {message.type === "ai" && (
                            <div className={`flex items-center justify-between mt-3 pt-2 border-t ${dark ? 'border-slate-700' : 'border-gray-100'}`}>
              ...
              {aiLoading && (
                <div className="flex justify-start">
                  <div className={`${dark ? 'bg-slate-800 border border-slate-700 text-gray-100' : 'bg-white border border-emerald-100'} rounded-2xl rounded-bl-none p-3 sm:p-4 max-w-[80%] shadow-md flex flex-col gap-2`}>
                    <div className="flex items-center gap-2 sm:gap-3">
              ...
              {aiError && (
                <div className="flex justify-start">
                  <div className={`${dark ? 'bg-red-900/20 border border-red-800 text-red-200' : 'bg-red-50 border border-red-200'} rounded-2xl rounded-bl-none p-3 sm:p-4 max-w-[80%] shadow-md`}>
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className="bg-red-500 p-1.5 rounded-lg">
                        <X size={14} className="text-white" />
                      </div>
                      <div className="flex-1">
                        <p className={`${dark ? 'text-red-200' : 'text-red-800'} text-sm font-medium`}>
                          Unable to process request
                        </p>
                        <p className={`${dark ? 'text-red-300' : 'text-red-600'} text-xs sm:text-sm mt-1`}>{aiError}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Input Area */}
        <div className={`border-t p-3 sm:p-6 shadow-lg ${dark ? 'border-slate-700 bg-slate-800 text-gray-100' : 'border-emerald-100 bg-white'}`}>
          <div className="flex gap-2 sm:gap-3">
            <input
              ref={inputRef}
              type="text"
              className={`flex-1 border-2 rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:outline-none focus:ring-2 text-sm sm:text-base font-medium transition-all duration-200 ${
                dark
                  ? "bg-slate-800 text-gray-100 placeholder-gray-400 border-slate-600 focus:border-slate-500 focus:ring-slate-500"
                  : "bg-white text-gray-800 placeholder-gray-500 border-emerald-200 focus:border-emerald-500 focus:ring-emerald-200"
              }`}
              placeholder={
                usage.uses >= usage.max
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mt-3">
            <button
              onClick={clearChat}
              className={`text-xs ${dark ? 'text-gray-400 hover:text-emerald-300' : 'text-gray-500 hover:text-emerald-700'} transition font-medium flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-gray-100`}
            >
              <span>🗑️</span>
              Clear conversation
            </button>
            <div className={`flex items-center gap-3 text-xs ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
              <div className={`font-medium ${dark ? 'bg-slate-700' : 'bg-gray-100'} px-2 py-1 rounded-lg`}>
